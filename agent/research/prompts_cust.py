...
- **Output**: Use ONLY content that has explicit source [Source: 文件名, Page X]. Do NOT use "Based on the knowledge base" as fallback. If the tool returns content without filename/page citation, do not present it—treat as unavailable.
...
**Step 6 – Synthesize**: 
Combine only content with explicit citations. Output per B and F (compact, mobile-friendly). Omit content without [Source: 文件名, Page X] (see A).
